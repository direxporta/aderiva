...
export function Product({ liked, toggleLiked, onAddToCart }: Props) {
  ...
  const localized = getProductCopy(index, language); const gallery = product.gallery.length ? product.gallery.slice(0,6) : [product.image]; const isLiked = liked.includes(index); const lowStock = product.stock <= 10;
  ...
  const categoryPath=product.category==='WOMEN'?'women':product.category==='MEN'?'men':'unisex'; const previousImage=()=>setActiveImage(c=>(c-1+gallery.length)%gallery.length); const nextImage=()=>setActiveImage(c=>(c+1)%gallery.length);
  return <>
    <section className="shop-section product-page-clean">
      ...
      <div className="product-detail-section size-chart-section"><details className="size-chart"><summary>Size chart & fit guide</summary><div className="size-chart-table"><table><thead><tr><th>{t.size}</th><th>Chest (cm)</th><th>Length (cm)</th><th>Shoulder (cm)</th></tr></thead><tbody>{[['XS','92','66','42'],['S','96','68','44'],['M','100','70','46'],['L','104','72','48'],['XL','108','74','50'],['XXL','112','76','52']].map(r=><tr key={r[0]}>{r.map(c=><td key={c}>{c}</td>)}</tr>)}</tbody></table><p className="size-chart-note">All measurements are approximate and based on the garment laid flat.</p></div></details></div>
      {pairWithProducts.length>0&&<section className="shop-section" style={{paddingTop:'40px',paddingBottom:'20px'}}><div className="section-heading"><div><p className="eyebrow">COMPLETE THE LOOK</p><h2>Pair this <em>with.</em></h2></div></div><div className="product-grid">{pairWithProducts.map(({originalIndex,product:p})=><ProductCard key={originalIndex} product={p} index={originalIndex} liked={liked.includes(originalIndex)} onLike={()=>toggleLiked(originalIndex)} onAdd={onAddToCart}/>)}</div></section>}
